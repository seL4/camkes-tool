 ...
 * @TAG(DATA61_BSD)
 *)
theory simple_CDL_Refine
imports
  simple_Arch_Spec (* generated arch spec *)
  simple_CDL (* generated CDL spec *)
...
abbreviation "arch_spec \<equiv> simple_Arch_Spec.assembly'"
abbreviation "cdl_state \<equiv> simple_CDL.state"
...
simproc_setup simple_CDL_id_equality ("(x :: cdl_object_id) = y") = \<open>
  fn _ => fn ctxt => fn ct =>
    case Thm.term_of ct of
        Const (@{const_name HOL.eq}, _) $ Const (id1, _) $ Const (id2, _) =>
            if String.isPrefix "simple_CDL." id1 andalso String.isSuffix "_id" id1
               andalso String.isPrefix "simple_CDL." id2 andalso String.isSuffix "_id" id2
            then SOME (Simplifier.rewrite (ctxt addsimps @{thms simple_CDL.ids}) ct)
            else NONE
      | _ => NONE
\<close>
...
definition simple_shared_objs :: "cdl_object_id set"
  where
  "simple_shared_objs \<equiv> {
      simple_ep_id
   }"

definition client_echo_shared_objs :: "cdl_object_id set"
  where
  "client_echo_shared_objs \<equiv>
      simple_shared_objs"
...
definition this_admissible_labelling :: "label agent_map \<Rightarrow> bool"
  ...
  "this_admissible_labelling label_of \<equiv>
      ...
      \<and> (\<forall>cap \<in> ran echo_cnode_caps. \<forall>i \<in> cap_objects cap.
              i \<notin> client_echo_shared_objs \<longrightarrow> label_of i = ''echo'')
      ...
      \<and> (\<forall>frame_i \<in> mapped_frames_of objects echo_group_bin_pd_caps.
             frame_i \<notin> client_echo_shared_objs \<longrightarrow> label_of frame_i = ''echo'')

      \<and> label_of client_cnode_id = ''client''
      \<and> label_of client_6_0_control_9_tcb_id = ''client''
      \<and> (\<forall>cap \<in> ran client_cnode_caps. \<forall>i \<in> cap_objects cap.
              i \<notin> client_echo_shared_objs \<longrightarrow> label_of i = ''client'')
      \<and> (\<forall>cap \<in> ran client_6_0_control_9_tcb_caps. \<forall>i \<in> cap_objects cap. label_of i = ''client'')
      \<and> (\<forall>pt_i \<in> mapped_pts_of objects client_group_bin_pd_caps. label_of pt_i = ''client'')
      \<and> (\<forall>frame_i \<in> mapped_frames_of objects client_group_bin_pd_caps.
             frame_i \<notin> client_echo_shared_objs \<longrightarrow> label_of frame_i = ''client'')

      \<and> (\<forall>i \<in> simple_shared_objs. label_of i = ''simple'')
      )"

subsection \<open>Evaluate cap sets\<close>

schematic_goal client_mapped_pts:
  "mapped_pts_of objects client_group_bin_pd_caps = ?val"
...
schematic_goal client_mapped_frames:
  "mapped_frames_of objects client_group_bin_pd_caps = ?val"
  apply (clarsimp simp: mapped_frames_of_def client_mapped_pts)
...
schematic_goal this_admissible_def':
  ...
  apply (clarsimp simp: client_mapped_pts client_mapped_frames
                        echo_mapped_pts echo_mapped_frames)
  apply (clarsimp simp: cap_defs objects obj_defs object_slots_def
                        client_echo_shared_objs_def
                        simple_shared_objs_def)
...
definition this_policy
  where
  "this_policy \<equiv> policy_of simple_Arch_Spec.assembly'"
...
definition this_admissible_pas :: "label PAS \<Rightarrow> bool"
  ...
  "this_admissible_pas pas \<equiv>
     ...
     pasSubject pas \<in> fst ` set (components (composition simple_Arch_Spec.assembly')) \<and>
...
lemma this_admissible_pas_exists:
  ...
  apply (rule_tac x = "undefined\<lparr>
                         ...
                         pasSubject := fst (hd (components (composition simple_Arch_Spec.assembly')))
                         \<rparr>"
                  in exI)
...
lemma this_asid_policy_trivial:
  ...
  apply (clarsimp simp: simple_CDL.state_def simple_CDL.asid_table_def
                        opt_cap_def slots_of_def opt_object_def object_slots_def)
  apply (erule cdl_state_asids_to_policy_aux.cases)
    (* ASIDs in CDL heap*)
    (* unfold object map *)
    apply (clarsimp simp: simple_CDL.objects_def)

    (* traverse mapping *)
    apply (erule case_obj_helper,
           time_methods \<open>solves \<open>clarsimp simp: simple_CDL.obj_defs simple_CDL.cap_defs
                                          split: if_split_asm\<close>\<close>)+
    apply (solves \<open>clarsimp simp: simple_CDL.empty_irq_objects_def simple_CDL.empty_irq_node_def
...
lemma this_irq_policy_trivial:
  ...
  apply (clarsimp simp: simple_CDL.state_def simple_CDL.irqs_def
                        opt_cap_def slots_of_def opt_object_def object_slots_def)
  (* unfold object map *)
  apply (clarsimp simp: simple_CDL.objects_def)
...
theorem this_pcs_refined:
  ...
  prefer 2
   apply (fastforce simp: simple_CDL.state_def simple_CDL.cdt_def)
  ...
  apply (time_methods
           cap_non_container_tac:
               \<open>erule case_obj_helper,
                solves \<open>simp (no_asm_use) add: object_slots_def simple_CDL.obj_defs\<close>\<close>
        |time_methods
           cap_to_container_tac:
               \<open>erule case_obj_helper,
                solves \<open>clarsimp simp: object_slots_def simple_CDL.cap_defs simple_CDL.obj_defs;
  ...
  apply (clarsimp simp: simple_CDL.empty_irq_objects_def simple_CDL.empty_irq_node_def
                        object_slots_def)
